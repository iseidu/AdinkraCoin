# Bootstrappable Adinkracoin Core Builds

This directory contains the files necessary to perform bootstrappable Adinkracoin
Core builds.
...
To build Adinkracoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Adinkracoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/adinkracoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/adinkracoin-core/guix.sigs).
