...
""" Interactive adinkracoind P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for Adinkracoin Core can enable. It uses BCC
...
def main(adinkracoind_path):
    peers = dict()

    adinkracoind_with_usdts = USDT(path=str(adinkracoind_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    adinkracoind_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    adinkracoind_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[adinkracoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/adinkracoind")
        exit()
    path = sys.argv[1]
    main(path)
